...
BEGIN 
 ...
 Parte1 AS 
 (      
  SELECT   pc.pco_evento as eventoDebito, 
	pc.pco_eventoCredito as eventoCredito,      
	mov_unidadeDestino,      
	mov_unidadeOrigem,      
	mov.uni_id as unidadeId,      
  ...
  --    (Case         
  --     when (Pmeio) = unGua.uni_id       
  --      then 'Debito'       
  --     when (select top 1 uni_id from dbo.FN_GetUnidadesSuperiores(mov_unidadeOrigem )) = unGua.uni_id       
  ...
  --************************************************************************      
   ...
   (select uni_codigoUGE from Unidade with (nolock) where uni_id = MOV.uni_id) as UGE2        
--unGua.uni_id,        
 -- uadGua.uad_nome        
 ...
 --   ON unGua.uni_id = mov.uni_id     
	--or unGua.uni_id = mov.mov_unidadeDestino      
...
Destino AS ( SELECT uni_id, uad_id , case when @uniges_id=85 then 1 else 0 end  as uge       
  FROM        
   dbo.FN_GetUnidadeEFilhas(@uniges_id)),

Retorno as (

	select top 100 eventoDebito, eventoCredito,mov_unidadeDestino,
	mov_unidadeOrigem,	cco_codigoCredito, cco_codigoDebito, 
	data, bem_id, bit_id, cco_codigo, unGua.uni_id,  uadGua.uad_nome  
	from parte1 p
	INNER JOIN tbUnidades AS unGua with (nolock)        
	ON unGua.uni_id = p.unidadeId     
		or unGua.uni_id = p.mov_unidadeDestino      
	INNER JOIN dbo.VW_UnidadeAdministrativa AS uadGua WITH(NoLock)        
	ON unGua.uad_id = uadGua.uad_id   
	where
	((@uniguarda_id  = 0) OR (unGua.uni_id = @uniguarda_id))        
	and  p.mov_unidadeDestino is not null   
)

select * from Retorno

/*
select eventoDebito, eventoCredito, 
mov_unidadeDestino, 
mov_unidadeOrigem,
cco_codigoCredito ,
cco_codigoDebito ,
